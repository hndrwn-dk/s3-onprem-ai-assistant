...
class FastSearch:
    ...
    def _get_relevant_snippet(
        self, content: str, query_words: List[str], snippet_length: int = 500
    ) -> str:
...
def quick_answer(query: str) -> str:
    ...
    # Format response
    response_parts = [
        f"Based on your S3 documentation (found in {len(results)} files):"
    ]
    ...
    response_parts.append(
        f"Matched keywords: {', '.join(set(word for r in results for word in r['matches']))}"
    )
