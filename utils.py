...
def ensure_documents_for_embedding() -> bool:
    ...
    real_docs = [
        doc
        for doc in docs
        if not doc.metadata.get("source", "").endswith(
    ...
    if len(real_docs) == 0:
        logger.warning(
            "No real documents found for embedding - only sample file exists"
        )
        logger.info(
            "Please upload PDF/TXT/MD/JSON files to the 'docs' folder before building embeddings"
        )
        return False
